...
export default function DashboardLayout({
  ...
  useEffect(() => {
    ...
    if (status === "unauthenticated") {
      console.log(`🔴 [L6] DashboardLayout: No autenticado, redirigiendo a login`);
      window.location.href = "/login";
    }
    
    if (status === "authenticated") {
      console.log(`✅ [L7] DashboardLayout: Autenticado correctamente`);
    }
  }, [status]);

  if (status === "loading") {
    console.log(`🟡 [L8] DashboardLayout: Cargando...`);
    return (
  ...
  if (!session) {
    console.log(`🔴 [L9] DashboardLayout: No hay sesión`);
    return null;
  }
