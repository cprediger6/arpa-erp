...
export async function PUT(
  ...
  // Solo ADMIN puede editar monedas
  if (session.user.role !== "ADMIN") {
    return NextResponse.json(
      { error: "No tienes permisos para editar monedas" },
      { status: 403 }
    );
  }
...
export async function DELETE(
  ...
  // Solo ADMIN puede eliminar monedas
  if (session.user.role !== "ADMIN") {
    return NextResponse.json(
      { error: "No tienes permisos para eliminar monedas" },
      { status: 403 }
    );
  }
